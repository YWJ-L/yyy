...
void ANO_Stick_Scan(void)   //左油门
{
#ifdef American_Mode                                                               //跑这里的程序
	Rc.THR = 1000 + (uint16_t)(0.25f*ADC_ConvertedValue[3]) - ANO_Param.OffSet_Thr;  //方向前后
	Rc.PIT = 1000 + (uint16_t)(0.25f*ADC_ConvertedValue[1]) - ANO_Param.OffSet_Pit;  //油门
#else																																							 //这里不工作
	Rc.PIT = 1000 + (uint16_t)(0.25f*ADC_ConvertedValue[1]) - ANO_Param.OffSet_Pit;
	Rc.THR = 2000 - (uint16_t)(0.25f*ADC_ConvertedValue[3]) - ANO_Param.OffSet_Thr;
#endif

	Rc.YAW = 1000 + (uint16_t)(0.25f*ADC_ConvertedValue[2]) - ANO_Param.OffSet_Yaw;//航向旋转
	Rc.ROL = 2000 - (uint16_t)(0.25f*ADC_ConvertedValue[0]) - ANO_Param.OffSet_Rol;//方向左右
